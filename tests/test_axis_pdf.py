...
def main():
	pdf_path = "data/raw/2025/08/axis/bank/AXIS BANK Statement for August 2025.pdf"
	if not os.path.exists(pdf_path):
		print(f"File not found: {pdf_path}")
		return
	text = extract_text_from_pdf(pdf_path)
	temp_txt_path = save_extracted_text(pdf_path, text)
	parser = AxisBankStatementParser(temp_txt_path)
	df = parser.parse()
	df_std = standardize_transactions(df, source="Axis Bank", is_credit_card=False)
