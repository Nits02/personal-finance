...
def main():
	pdf_path = "data/raw/2025/08/icici/credit_card/ICICI Credit Card Statement.pdf"
	if not os.path.exists(pdf_path):
		print(f"File not found: {pdf_path}")
		return
	text = extract_text_from_pdf(pdf_path)
	temp_txt_path = save_extracted_text(pdf_path, text)
	parser = ICICICreditCardParser(temp_txt_path)
	df = parser.parse()
	df_std = standardize_transactions(df, source="ICICI Credit Card", is_credit_card=True)
